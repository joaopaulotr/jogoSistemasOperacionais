...
- Implementar framing de mensagens (tamanho + payload) para garantir entrega correta de JSONs grandes.
- Adicionar reconexão/tempo limite e tratamento de desconexões do cliente.
- Implementar UI mais amigável (ex.: web, GUI) e suporte a múltiplas partidas simultâneas.
- Adicionar testes automatizados unitários para as funções de validação e lógica de ataque.
...
Este projeto está licenciado – ver arquivo `LICENSE`.

Boa diversão jogando! Se quiser, posso também melhorar o protocolo (por exemplo, usar framing) ou criar uma versão com interface web.
